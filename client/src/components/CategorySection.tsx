  ...
  return (
    <section id={category.slug} className="mb-6 sm:mb-8 md:mb-10 lg:mb-12">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5 md:mb-6 space-y-2 sm:space-y-0">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#efbf04] flex items-center">
          <span 
            className="w-1 h-6 sm:h-7 md:h-8 mr-2 sm:mr-3 bg-[#efbf04]"
          ></span>
          {category.name}
        </h2>
        <Link
          href={`/category/${category.slug}`}
          className="text-sm sm:text-base font-semibold flex items-center transition-colors hover:underline self-start sm:self-center text-[#efbf04]"
        >
